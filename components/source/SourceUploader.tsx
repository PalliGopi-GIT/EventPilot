...
import { Upload, FileText, Image as ImageIcon, FileCode, AlertCircle, Loader2, Sparkles, ArrowRight } from "lucide-react";
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-800">
        <div>
          <h2 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
            ...
            1. Ingest Event Source
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            Feed event poster, PDF brochure, presentation, or raw announcement to Agent 1
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {activeTab === "file" ? (
          <div>
            ...
            >
              ...
              {selectedFile ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  <div className="p-4 bg-slate-800/80 rounded-full text-blue-400">
                    <Upload className="w-8 h-8" />
                  </div>
                  <div>
                    <p className="text-sm font-medium text-slate-200">
                      Drop event poster, flyer, PDF brochure or PPT here
                    </p>
                    <p className="text-xs text-slate-400 mt-1">
                      Supports PNG, JPG, PDF, PPTX, TXT up to 15MB
                    </p>
                  </div>
                  <span className="text-xs px-3 py-1 bg-slate-800 rounded-md text-slate-300 border border-slate-700">
                    Browse Files
                  </span>
                </div>
              )}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-2">
              Event Announcement / Schedule Text
            </label>
            <textarea
              rows={6}
              value={rawText}
              onChange={(e) => setRawText(e.target.value)}
              placeholder="Paste email invitation, WhatsApp notice, meetup description, or conference schedule here...&#10;&#10;Example:&#10;🚀 AI Agents Workshop 2026&#10;Date: October 15, 2026 | 10:00 AM&#10;Venue: Main Auditorium, CS Department&#10;Topics: Multi-agent systems, GLM reasoning, Tool execution&#10;Organized by: IEEE CS Chapter"
              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs text-slate-200 focus:outline-none focus:border-blue-500 font-mono resize-y"
            />
          </div>
        )}
        ...
        <div className="flex justify-end pt-2">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-lg shadow-md shadow-blue-600/20 transition-all cursor-pointer"
          >
            {isLoading ? (
              <>
